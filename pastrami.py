...
class Analysis :
    ...
    def __init__(self, opts, unknown_args) :
        ...
        if self.sub_command == 'build' :
            ...
            self.reference_tpeds = pd.Series()
            self.reference_background = pd.Series()
    ...
    def validate_options(self) :
        ...
        if self.sub_command == 'coanc' :
            self.validate_reference_prefix()
            if not self.query_prefix is None :
                self.validate_query_prefix()
            self.validate_haplotypes()
    ...
    def pull_chromosome_tped(self, prefix, chromosome) :
        ...
        command = ' '.join(['rm', ' '.join([chromosome_file_prefix + '.' + i for i in ['tped', 'tfam']])])
        print(command)
        subprocess.call(command, shell = True)
...
#        self.haplotypes = self.haplotypes.iloc[self.haplotypes.iloc[:,0].values in self.chromosomes, :]
        ...
        chunk_data = [[chunk_indices[i]] + [chunk_haplotypes[i]] + [chunk_genotypes[i]] + [i] for i in range(len(chunk_indices))]
    ...
    def chunk_build_reference_copying_fractions(self, chunk_data) :
        ...
        for haplotype in range(len(chunk_haplotypes)) :
            ...
            haplotype_counts = pd.DataFrame({'population': self.reference_tfam.loc[haplotype_strings.index, 'population'],
            ...
            haplotype_counts_with_zeroes = pd.DataFrame(0, index = distinct_haplotypes, columns = self.reference_populations)
            ...
            for individual in self.reference_individuals :
                ...
                if not individual in haplotype_strings.index :
                    continue
...
#                print(individual)
    ...
    def query_reference_set(self) :
        ...
        chunk_data = [[chunk_indices[i]] + [chunk_haplotypes[i]] + [chunk_genotypes[i]] + [chunk_reference_fractions[i]] + [i] for i in range(len(chunk_indices))]
        ...
        self.combined_copying_fractions = pd.concat([self.reference_copying_fractions, self.query_copying_fractions], axis = 0)
    ...
    def build_coanc(self) :
        ...
        for chromosome in self.chromosomes :
            ...
            print('Splitting haplotypes into chunks of', chunk_size)
            chunk_indices = list(range(0, math.ceil(chromosome_haplotypes.shape[0] / chunk_size) * chunk_size + 1, chunk_size))
            chunks = len(chunk_indices) - 1
    ...
    def chunk_build_coanc(self, chunk_data) :
        ...
        chunk_haplotype_fractions = pd.Series()
        for individual in self.reference_individuals :
            chunk_haplotype_fractions[individual]  = pd.Series([0] * len(self.reference_individuals), index = self.reference_individuals)
        ...
        for haplotype in range(len(chunk_haplotypes)) :
            ...
            distinct_haplotypes = haplotype_strings.unique()
            haplotype_counts_with_zeroes = pd.DataFrame(0, index = distinct_haplotypes, columns = self.reference_individuals)
            haplotype_counts_with_zeroes.loc[haplotype_counts.index, haplotype_counts.columns] += haplotype_counts
            haplotype_counts = haplotype_counts_with_zeroes
...
#            print(haplotype_counts.head())
            ...
            for individual in haplotype_reference_individuals :
                ...
                if individual_haplotype in haplotype_counts.index :
                    chunk_haplotype_fractions[individual] = chunk_haplotype_fractions[individual] + haplotype_counts.loc[individual_haplotype, :]
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(prog = "chromofake.py",
                            add_help = False,
                            description =
                            '''
                            Chromofake - population scale haplotye copying
                            ''',
                            formatter_class = lambda prog: HelpFormatter(prog, width = 120, max_help_position = 120))

    parser.add_argument('--help', action = 'store_true', default = False)

    subparsers = parser.add_subparsers(title = 'Tequila commands')
    ...
    ## And the co-ancestry sub-command
    ...
    if opts.sub_command == 'build' and opts.help :
        print(Colors.HEADER)
        build_parser.print_help()
        print(Colors.ENDC)
        sys.exit(0)

    if opts.sub_command == 'query' and opts.help :
        print(Colors.HEADER)
        query_parser.print_help()
        print(Colors.ENDC)
        sys.exit(0)

    if opts.sub_command == 'coanc' and opts.help :
        print(Colors.HEADER)
    ...
    if len(analysis.errors) > 0 :
        ...
        if opts.sub_command == 'build' :
            build_parser.print_help()
        elif opts.sub_command == 'query' :
            query_parser.print_help()
        elif opts.sub_command == 'coanc' :
            coanc_parser.print_help()
        print(Colors.ENDC)
        print(Colors.FAIL + '\n\nErrors:')
